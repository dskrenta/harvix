		...
		foreach my $result ( @{ $results } )
			...
			if ($count < 6)
			{
				if ( $img_data ne '' )
						...
						<p></p>
                                                <form action="summarize.php" method="GET" target="_blank"> 
						<input type="hidden" name="q" value="$url"> 
						<button type="submit" class="btn btn-small btn btn-info">Summarize</button> 
						</form>
						</div> <!--1-->
                                                <div class="span4">
...
<div id='nav-bar-top'>
  ...
  <form id="searchForm" class="form-wrapper cf" onsubmit="submitted('h'); return false" action="http://harvix.com/search/s.cgi" name="searchForm">
