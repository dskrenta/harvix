...
{
    foreach my $ad ( @{ $xml->{'SITE'} } )		# for every advertisement that 7search returned to us
    {
	if ($count == 0)
	{
		print "<a href=\"$ad->{'URL'}\">$ad->{'NAME'}</a><p>$ad->{'DESCRIPTION'}<p><span style=\"color:green\">$ad->{'HTTPLINK'}</span>\n";
		print "<hr></hr><p>\n";
	}
