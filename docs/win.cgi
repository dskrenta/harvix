...
print"<td width=\"90%\";>";
 ...
 my $agent = WWW::Google::Images->new(
    ...
    my $result = $agent->search(($query), limit => 200);


print"<div class=\"h10\"> <div class=\"large\"><b><i>Image results for \"$query\"</i></b></div><p>";
...
my $count = 0;
  ...
  while (my $image = $result->next()) {
	...
	my $url = $image->content_url();
        print("<A HREF=\"$url\"><IMG WIDTH=\"\" HEIGHT=\"300\"SRC=\"$url\" ALT=\"$url\"/></A>\n");
...
if ($count >= 20)
                {
        my $url = $image->content_url();
        print("<A HREF=\"$url\"><IMG WIDTH=\"\" HEIGHT=\"250\"SRC=\"$url\" ALT=\"$url\"/></A>\n");
...
print"Images powered by Google Images\n";
...
<td>
<form id="searchForm" name="searchForm" action="http://harvix.com/harviximages.cgi" onsubmit="submitted('h'); return false">
...
<td>
<div class="side">
<a href="http://harvix.com/hsearch1.cgi?q=$query">Everything</b>
<p>
