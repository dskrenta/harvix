...
foreach my $tag ( @{ $json->{tags} } )
{
	...
	my $num = scalar @{ $results };
    	print "<div id=\"count\"><span class=\"label\"><h3>$num results:</h3></span></div> <div id=\"upper\"><span class=\"label label-success\"><h3>Top Results:</h3></span></div><hr></hr>";
	}

	elsif($category eq "WIKI")
        {
        my $num = scalar @{ $results };
        print "<div id=\"upper\"><span class=\"label\"><h3>Quick Information:</h3></span></div><hr></hr>";
        }
	...
	{
	my $num = scalar @{ $results };
	print "<div id=\"count\"><span class=\"label\"><h3>$num results:</h3></span></div> <div id=\"upper\"><span class=\"label label-important\"><h3>Images:</h3></span></div><hr></hr>";
	}
	
	else
	{
	my $num = scalar @{ $results };
        print "<div id=\"count\"><span class=\"label\"><h3>$num results:</h3></span></div> <div id=\"upper\"><span class=\"label label-info\"><h3>$category:</h3></span></div><hr></hr>";
	}
	...
	if($category eq "IMAGE")	
	{
		...
		foreach my $result ( @{ $results } )
			...
			my $url = $result->{'u'};        
        		print"<img src=\"$url\" height=\"395px\"/>";
			print"</div>";
		}
	...
	elsif($category eq "WIKI")
	{
		...
		print"<iframe src=\"http://harvix.com/mswolf2.cgi?$query\" frameborder=\"0\"  allowTransparency=\"true\" width=\"100%\" height=\"300px\" scrolling=\"no\"></iframe>";
        	print"</div></div>";
	}
	}
	...
	my $count = 0;
		...
		if($count < 10)
		{
                	...
                	my $result = $agent->search(($rurl), limit => 1);
                	while (my $image = $result->next())
                	{
                        	my $hihi = $image->content_url();
                        	print"<img height=\"100px\"  src=\"$hihi\"/>";
                        	print"<h3>$title</h3>";
                        	print"$snippet";
                        	print"<p><span style=\"color:green\">$rurl</span>";
	                }
...
text-transform:uppercase;
	...
	<link rel="apple-touch-icon-precomposed" sizes="144x144" href="http://www.harvix.com/images/hiconnew2.jpg">
    <link rel="apple-touch-icon-precomposed" sizes="114x114" href="http://www.harvix.com/images/hiconnew2.jpg">
      <link rel="apple-touch-icon-precomposed" sizes="72x72" href="http://www.harvix.com/images/hiconnew2.jpg">
                    <link rel="apple-touch-icon-precomposed" href="http://www.harvix.com/images/hiconnew2.jpg">
                                   <link rel="shortcut icon" href="http://www.harvix.com/images/hiconnew2.jpg">
